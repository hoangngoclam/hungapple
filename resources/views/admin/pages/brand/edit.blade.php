...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    <form id="form" action="{{ route('admin.editBrandPost') }}" method="POST"
                        enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <input type="hidden" value="{{ $brand->id }}" name="id">
                        <div class="form-body">
                            <h5 class="card-title">Thông tin brand</h5>
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="name"
                                            class="form-control
                                    {{ $errors->slider->first('name') ? 'is-invalid' : '' }}"
                                            placeholder="Nhập vào" name="name"
                                            value="{{ old('name') ? old('name') : $brand->name }}" required>
                                        @if ($errors->slider->first('name'))
                                            <div class="invalid-feedback">
                                                {{ $errors->slider->first('name') }}
                                            </div>
                                        @endif
                                    </div>
                                </div>
                            ...
                            <div>
                                ...
                                <div class="row" id="imagesPreview"> <a href="#" class="preview-image">
                                        <img class="img-fluid img-thumbnail" src="{{ url($brand->image) }}"
                                            alt="{{ url($brand->image) }}">
                                    </a></div>
